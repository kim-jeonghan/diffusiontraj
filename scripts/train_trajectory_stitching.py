...
from comp_diffuser.models.diffusion.trajectory_stitching_diffusion import (
    StitchingDiffusion,
)
...
def main():
    ...
    if not isinstance(diffusion_model, StitchingDiffusion):
        raise ValueError(
            "`train_trajectory_stitching.py` requires a stitching config that builds "
            f"`StitchingDiffusion`, but got `{type(diffusion_model).__name__}`. "
            "Do not pass a baseline Maze config here."
        )
