...
def main(args_train, args):
    ...
    maze_planner = TrajectoryStitchingMazePlanner(args_train, args=args)
    maze_planner.setup_load(ld_config=ld_config)
...
if __name__ == "__main__":
    ...
    if latest_e < 0:
        raise RuntimeError(
            "No trained baseline checkpoint found. "
            f"Available epochs at {osp.join(*loadpath)}: {available_epochs}. "
            "Run baseline training until it saves a checkpoint, "
    ...
    if get_checkpoint_step(loadpath, latest_e) <= 0:
        raise RuntimeError(
            "No trained baseline checkpoint found. "
            f"Available epochs at {osp.join(*loadpath)}: {available_epochs}. "
            "Run baseline training until the latest checkpoint records a "
            "positive training step, or point planning at the correct trained "
            "artifact directory."
        )
