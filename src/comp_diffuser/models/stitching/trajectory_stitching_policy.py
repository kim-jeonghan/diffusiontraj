...
class TrajectoryStitchingPolicy:
    ...
    def generate_conditioned_trajectory(
        ...
        if self.num_segments == 1:
            return self.generate_single_segment_trajectory(
                planner_inputs, b_s=batch_size
            )
        ...
        boundary_conditions = {
            0: einops.repeat(
                start_goal_pairs[0, :, :], "n_p d -> (n_p rr) d", rr=batch_size
            ).clone(),
            horizon
            - 1: einops.repeat(
                start_goal_pairs[1, :, :], "n_p d -> (n_p rr) d", rr=batch_size
            ).clone(),
        }
        ...
        if self.inference_schedule == "interleaved":
            ...
            segment_trajectories = (
                self.diffusion_model.comp_pred_p_loop_n_same_t_parallel(
                    sample_shape,
                    boundary_conditions,
                    n_comp=self.num_segments,
                    return_diffusion=False,
    ...
    def generate_single_segment_trajectory(
        self, planner_inputs, debug=False, batch_size=1
    ):
        """
        ...
        num_problems = start_goal_pairs.shape[1]
        assert (
            start_goal_pairs.ndim == 3
            and start_goal_pairs.shape[0] == 2
            and num_problems == 1
        )
        ...
        predicted_trajectories = self.diffusion_model.conditional_sample(
            g_cond=diffusion_inputs
        )

        predicted_trajectories = apply_conditioning(
...
class TrajectoryStitchingPrediction:
    def __init__(self, pick_traj, trajs_list_topn_bl, trajs_list_np_un) -> None:
        """
