...
class TrajectoryStitchingGaussianDiffusionWithInverseDynamics(nn.Module):
    ...
    def p_sample(self, x, segment_conditioning, timesteps, mask_same_t=None):
        ...
        model_mean, _, model_log_variance = self.p_mean_variance(
            x=x,
            t_2d=timesteps,
            segment_conditioning=segment_conditioning,
        )
        ...
        if mask_same_t is not None:
            ...
            ## B,H
            assert (
                mask_same_t.shape == timesteps.shape and mask_same_t.dtype == torch.bool
            )
            ## TODO: temporally
            assert (timesteps[mask_same_t] == self.n_timesteps - 1).all() or (
                timesteps[mask_same_t] == 0
            ).all()
    ...
    def p_losses(
        ...
        if self.w_loss_type == "no_unused":
            ...
            batch_loss_w[segment_conditioning["uses_start_inpaint"], 0] = 0.0
            batch_loss_w[segment_conditioning["uses_end_inpaint"], self.horizon - 1] = (
                0.0
            )
            # pdb.set_trace()
    ...
    def create_train_conditioning(
        ...
        st_traj = self.q_sample(
            x_start=st_traj, t_2d=t_2d_st, noise=None, mask_no_noise=None
        )

        end_traj = x_clean[:, -self.len_ovlp_cd :, :].detach().clone()
        end_traj = self.q_sample(
            x_start=end_traj, t_2d=t_2d_end, noise=None, mask_no_noise=None
        )
    ...
    def create_eval_conditioning(
        ...
        if t_type == "rand":
            ...
            t_1d_st = t_1d_st - torch.ones_like(
                t_1d_st,
            )
            t_1d_end = t_1d_end - torch.ones_like(
                t_1d_end,
            )
        elif t_type == "0":
            pass
    ...
    def resample_same_t_mcmc(
        ...
        for i_mc in range(self.eval_n_mcmc):
            ...
            model_pred = self.model_predictions(
                x=x_t_mc, t_2d=timesteps, segment_conditioning=segment_conditioning
            )
    ...
    def comp_pred_p_loop_n_GSC(
        ...
        for i_t in tqdm(time_idx):
            ...
            x_p_list_next_t = [
                None for _ in range(n_comp)
            ]  ## after denoise this step, aka., less noisy
