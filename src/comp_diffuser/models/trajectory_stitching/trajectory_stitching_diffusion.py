...
from comp_diffuser.models.common.model_outputs import ModelPrediction
...
from comp_diffuser.models.hi_helpers import MLP_InvDyn
from comp_diffuser.models.trajectory_stitching import TrajectoryStitchingTemporalUNet


class TrajectoryStitchingGaussianDiffusionWithInverseDynamics(nn.Module):
    def __init__(self, model: TrajectoryStitchingTemporalUNet, 
    ...
    def p_mean_variance(self, x, t_2d, segment_conditioning, return_modelout=False):
        ...
        if segment_conditioning['use_conditioning']:
            ...
            x_2, t_2d_2, tj_cond_2 = utils.batch_repeat_tensor_in_dict(x, t_2d, segment_conditioning, n_rp=2)
    ...
    @torch.no_grad()
    def p_sample(self, x, segment_conditioning, timesteps, mask_same_t=None):
        """
        mask_same_t: bool tensor, (B,H); if True, the point remains same noise level
        """
        ## timesteps is 2D tensor: (B, H)
        b, *_, device = *x.shape, x.device
        model_mean, _, model_log_variance = self.p_mean_variance(x=x, t_2d=timesteps, segment_conditioning=segment_conditioning,)
    ...
    def get_segment_conditioning(self, x, g_cond, timesteps):
        ...
        if g_cond['conditioning_mode'] == 'overlap_only':
            ...
            x, segment_conditioning = self.create_eval_conditioning(
                ...
                t_type=g_cond['t_type'],
                is_noisy=False,
                stgl_cond={self.horizon-1: g_cond['boundary_conditions'][self.horizon-1] },
                )
            segment_conditioning['use_conditioning'] = True
    ...
    def p_sample_loop(self, shape, g_cond, verbose=True, return_diffusion=False):
        ...
        for i in tqdm(reversed(range(0, self.n_timesteps))):
            ...
            x, segment_conditioning = self.get_segment_conditioning(x, g_cond, timesteps)
    ...
    def conditional_sample(self, g_cond, *args, horizon=None, **kwargs):
        ...
        #     batch_size = len(segment_conditioning['start_overlap_traj'])
        # elif  segment_conditioning['end_overlap_is_drop'] is not None:
        #     batch_size = len(segment_conditioning['end_overlap_traj'])
        # else:
        #     assert False
        # pdb.set_trace()
    ...
    def p_losses(self, x_start, x_noisy, noise, t_2d, segment_conditioning, mask_no_noise=None, batch_loss_w=None,):
        ...
        ## Make sure segment_conditioning is totaly ready here
        ...
        if self.w_loss_type == 'no_unused':
            ...
            batch_loss_w[segment_conditioning['uses_start_inpaint'], 0] = 0.
            batch_loss_w[segment_conditioning['uses_end_inpaint'], self.horizon-1] = 0.
            # pdb.set_trace()
        ...
        x_recon = self.small_model_pred(x_noisy, t_2d, segment_conditioning)
        ## NOTE:
    ...
    def apply_cond_mask(self, x_recon, segment_conditioning):
        pass
    ...
    def create_train_conditioning(
        ...
        ####
        cond_end = {self.horizon-1: boundary_conditions[self.horizon-1][end_cd_use_inpat]}
        ...
        if is_rand:
            start_timesteps_1d = start_timesteps_1d - torch.randint_like(start_timesteps_1d, low=0, high=2)
            end_timesteps_1d = end_timesteps_1d - torch.randint_like(end_timesteps_1d, low=0, high=2)
        else:
            assert False
            # t_1d_st = t_1d.clone()
            # t_1d_end = t_1d.clone()
        ## Oct 6 newly Added
        start_timesteps_1d = torch.clamp(start_timesteps_1d, min=0, max=self.n_timesteps-1)
        end_timesteps_1d = torch.clamp(end_timesteps_1d, min=0, max=self.n_timesteps-1)


        ## TODO: slow, can be improved
        t_2d_st = torch.repeat_interleave(start_timesteps_1d[:, None], repeats=self.len_ovlp_cd, dim=1)
        t_2d_end = torch.repeat_interleave(end_timesteps_1d[:, None], repeats=self.len_ovlp_cd, dim=1)
        # pdb.set_trace()
    ...
    def loss(self, x_clean, cond_start_goal):
        ...
        if self.is_train_diff:
            ...
            # pdb.set_trace()
            diffuse_loss, info = self.p_losses(x_clean[:, :, :], x_noisy, noise, t_2d, segment_conditioning,
    ...
    def small_model_pred(self, x, t_2d, segment_conditioning: dict, ) -> torch.Tensor:
        ...
        if self.model.input_t_type == '1d':
            ...
            t_1d = t_2d[:, 0]
            pred_out = self.model(x, t_1d, segment_conditioning)
    ...
    def create_eval_conditioning(self, 
    ...
    def ddim_p_sample_loop(self, shape, g_cond, verbose=True, return_diffusion=False):
        ...
        for i in tqdm(time_idx):
            ...
            ## ----------------------
            ### eta=0.0
            x = self.ddim_p_sample(x, segment_conditioning, timesteps, self.ddim_eta, use_clipped_model_output=True)
            # x = apply_conditioning(x, cond, 0)
    ...
    @torch.no_grad()
    def ddim_p_sample(self, x, segment_conditioning, timesteps, eta=0.0, 
        ...
        model_mean, _, model_log_variance, x_recon, model_output = \
                self.p_mean_variance(x=x, t_2d=timesteps, segment_conditioning=segment_conditioning, return_modelout=True)
    ...
    def resample_same_t_mcmc(self, x: torch.Tensor,
            segment_conditioning, timesteps,):
        x_t_mc = torch.clone(x)
        for i_mc in range(self.eval_n_mcmc):
            ## 1. from x_t to x_0; x_0 shape (B, H, tot_hzn)
            model_pred = self.model_predictions(x=x_t_mc, t_2d=timesteps, segment_conditioning=segment_conditioning )
    ...
    def comp_pred_p_loop_n(
        ...
        assert num_segments >= 2 and not do_mcmc, 'mcmc might be bad in our case and not implemented for DDIM yet.'
        device = self.betas.device

        batch_size = shape[0]
        horizon = shape[1]

        segment_samples = [torch.randn(shape, device=device) for _ in range(num_segments)]
        ...
        for time_index in tqdm(time_idx):
            ...
            ## timesteps = torch.full((batch_size,), i, device=device, dtype=torch.long) # old
            ## e.g., (B=10,H=384)
            timesteps = torch.full((batch_size, self.horizon), time_index, device=device, dtype=torch.long)
            ...
            for segment_index in range(num_segments):
                ...
                if segment_index == 0:
                    ...
                    next_segment_start_overlap, _ = self.extract_ovlp_from_full(next_segment)

                    current_segment, segment_conditioning = self.create_eval_conditioning(
                    ...
                    segment_conditioning['use_conditioning'] = True
                    if do_mcmc:
                        current_segment = self.resample_same_t_mcmc(current_segment, segment_conditioning, timesteps)

                    if self.use_ddim:
                        current_segment = self.ddim_p_sample(current_segment, segment_conditioning, timesteps, self.ddim_eta, use_clipped_model_output=True)
                    else:
                        current_segment = self.p_sample(current_segment, segment_conditioning, timesteps)
                    ...
                    _, previous_segment_end_overlap = self.extract_ovlp_from_full(previous_segment)
        ...
        ## Finished
        segment_samples[0] = apply_conditioning(segment_samples[0], {0: boundary_conditions[0]}, 0)
    ...
    def comp_pred_p_loop_n_same_t_parallel(
        ...
        for i_t in tqdm(time_idx):
            ...
            ## ========= Finish setting up all the segment_conditioning ==========
            ...
            x_p_list_cur_t = einops.rearrange(x_p_list_cur_t, 
                                              '(num_segments B) h d -> num_segments B h d', num_segments=num_segments)
        ...
        ## Finished
        segment_samples[0] = apply_conditioning(segment_samples[0], {0: boundary_conditions[0]}, 0)
        segment_samples[-1] = apply_conditioning(segment_samples[-1], {horizon - 1: boundary_conditions[horizon - 1]}, 0)
            ...
            for i_tj in range(n_comp):
                ...
                if i_tj == 0:
                    ...
                    tj_cond_p_i = dict(
                        start_overlap_is_drop=None,
                        end_overlap_is_drop=None,
                        uses_start_inpaint=torch.zeros_like(x_p_i[:,0,0]).to(torch.bool),
                        uses_end_inpaint=torch.zeros_like(x_p_i[:,0,0]).to(torch.bool),
                    )
                    ...
                    # pdb.set_trace() ## TODO: Jan 24 Check Back all segment_conditioning
    ...
    def average_overlap_chunk_global_sync(self, segment_samples):
        ...
        if len(segment_samples) <= 2:
            ...
            for segment_index in range(1, len(segment_samples)):
                previous_overlap = segment_samples[segment_index - 1][:, -self.len_ovlp_cd:]
                current_overlap = segment_samples[segment_index][:, :self.len_ovlp_cd]
                averaged_overlap = (previous_overlap + current_overlap) / 2
                segment_samples[segment_index - 1][:, -self.len_ovlp_cd:] = averaged_overlap
    ...
    def comp_pred_p_loop_n_ar_backward(
        ...
        for i_t in tqdm(time_idx):
            ...
            ## iteratively denoise each sub traj in backward order
