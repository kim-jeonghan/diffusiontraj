...
class TrajectoryStitchingTrainer(object):
    ...
    def train(self, n_train_steps):
        ...
        for i_tr in range(n_train_steps):
            ...
            if self.sample_freq and self.step % self.sample_freq == 0:
                ...
                self.render_samples(n_samples=self.n_samples, conditioning_mode='overlap_only')
                self.render_samples(n_samples=self.n_samples, conditioning_mode='boundary_only')
                self.render_samples(n_samples=self.n_samples, conditioning_mode='start_boundary_end_overlap')
                self.render_samples(n_samples=self.n_samples, conditioning_mode='start_overlap_goal_boundary')
    ...
    def render_samples(self, batch_size=1, n_samples=2, conditioning_mode=None):
        ...
        for i in range(batch_size):
            ...
            elif conditioning_mode == 'start_boundary_end_overlap':

                traj_full = einops.repeat(batch.obs_trajs, 'b h d -> (repeat b) h d', 
                                          repeat=n_samples).to('cuda:0')
                g_cond = dict(conditioning_mode='start_boundary_end_overlap', traj_full=traj_full, t_type='rand', boundary_conditions=boundary_conditions)
                samples = self.ema_model.conditional_sample(g_cond=g_cond)
                
            elif conditioning_mode == 'start_overlap_goal_boundary':

                traj_full = einops.repeat(batch.obs_trajs, 'b h d -> (repeat b) h d', 
                                          repeat=n_samples).to('cuda:0')
                g_cond = dict(conditioning_mode='start_overlap_goal_boundary', traj_full=traj_full, t_type='rand', boundary_conditions=boundary_conditions)
                samples = self.ema_model.conditional_sample(g_cond=g_cond)

            elif conditioning_mode in [None, False]:
