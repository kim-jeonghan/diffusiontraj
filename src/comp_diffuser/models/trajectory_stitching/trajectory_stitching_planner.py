...
import comp_diffuser.datasets as datasets
...
class TrajectoryStitchingMazePlanner:
    ...
    def setup_load(self, ld_config):
        ...
        self.train_normalizer = utils.load_trajectory_stitching_dataset_normalizer(args_train,)

        self.diffusion: TrajectoryStitchingGaussianDiffusionWithInverseDynamics = diffusion_experiment.ema
        ...
        if self.diffusion.uses_inverse_dynamics:
            ...
            if diffusion_name == 'dd_maze':
                ...
                self.policy = MazePolicy(self.diffusion, self.train_normalizer,
                                         self.policy_config)
                ...
                self.policy = TrajectoryStitchingPolicy(self.diffusion, self.train_normalizer, 
    ...
    def load_ev_problems(self):
        ...
        self.problems_h5path = utils.get_trajectory_stitching_eval_problem_path(self.env.name)
        self.problems_dict = utils.load_trajectory_stitching_eval_problems(h5path=self.problems_h5path)        
        # pdb.set_trace()
        return
    ...
    def plan_once_parallel(self, pl_seed=None, given_probs=None): ## num_ep
        ...
        for i_ep in range(num_ep):
            ...
            if i_ep % self.n_batch_acc_probs == 0:
                ...
                g_cond = {
                    'start_goal_pairs': np.array([input_st_acc, gl_pos_acc], dtype=np.float32)
                }
                # pdb.set_trace() ## TODO: check above
                m_out_list, pick_traj_acc = self.policy.generate_conditioned_trajectories_parallel(
    ...
    def plan_once(self, pl_seed=None, given_probs=None): ## num_ep
        ...
        for i_ep in range(num_ep):
            ...
            for t in range(self.env.max_episode_steps):
                ...
                if t == 0:
                    ...
                    g_cond = {
                        'start_goal_pairs': np.array([input_st[None,], target[None,]], dtype=np.float32)
                    }
                    # pdb.set_trace()
        ...
        json_data = OrderedDict([
            ...
            ('trajectory_blender_config', getattr(self, 'trajectory_blender_config', {})),
