...
class TrajectoryStitchingTemporalUNet(nn.Module):
    ...
    def __init__(
        ...
        if network_config.get("ovlp_model_type", "unet") == "unet":
            ...
            raise NotImplementedError("dit_enc was removed in the maze2d-only trim")
        else:
        ...
        self.inpaint_token_type = self.network_config[
            "inpaint_token_type"
        ]  ## e.g., const
        ...
        assert (
            wall_embed_dim == self.ext_cond_dim
        )  ## seems useless below, can be delete?
    ...
    def forward(
        ...
        if st_ovlp_is_drop is not None:  ##
            st_ovlp_feat = self.st_ovlp_model(
                tj_cond["st_ovlp_traj"], time=tj_cond["st_ovlp_t"]
            )
            ...
            assert not torch.logical_and(
                ~st_ovlp_is_drop, is_st_inpat
            ).any()  ## must be false
        else:
            ## no cond if None
            # st_ovlp_feat = torch.zeros_like(st_ovlp_feat)
            st_ovlp_feat = torch.zeros(
                (x.shape[0], self.st_ovlp_model.out_dim), device=x.device
            )

        if tj_cond["end_ovlp_is_drop"] is not None:
            end_ovlp_feat = self.end_ovlp_model(
                tj_cond["end_ovlp_traj"], time=tj_cond["end_ovlp_t"]
            )
            end_ovlp_feat[tj_cond["end_ovlp_is_drop"]] = 0.0
            ...
            end_ovlp_feat = torch.zeros(
                (x.shape[0], self.end_ovlp_model.out_dim), device=x.device
            )
        ...
        if self.inpaint_token_type == "const":
            ...
            st_token = torch.zeros(
                size=(b_size, self.inpaint_token_dim), dtype=x.dtype, device=x.device
            )
            ...
            end_token[is_end_inpat] = self.end_use_inpaint_token.repeat(
                (num_end_inpt, 1)
            )
        ...
        if self.cat_t_w:
            ...
            t_feat = torch.cat(
                [t_feat, st_ovlp_feat, end_ovlp_feat, st_token, end_token], dim=-1
            )
