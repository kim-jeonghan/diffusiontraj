...
class ConditionalTemporalUNet(nn.Module):
    ...
    def __init__(
        ...
        utils.print_color(
            f"[ models/ConditionalTemporalUNet ] Channel dimensions: {in_out}", c="c"
        )
        ...
        if self.energy_mode:
            ...
            self.energy_param_type = network_config[
                "energy_param_type"
            ...
            print(
                f"[ models/temporal_cond ] conv_zero_init {self.energy_param_type} {self.conv_zero_init}"
            )
        else:
            mish = True
            act_fn = nn.Mish()
    ...
    def forward(
        ...
        if self.energy_mode:
            ...
            if not self.training:
                eps = torch.autograd.grad(
                    [energy_norm.sum()], [x_inp], create_graph=False
                )[0]
