...
class Traj_Blender:
    ...
    def blend_traj_lists(self, trajs_list, do_unnorm):
        ...
        tot_hzn = (
            n_comp * self.diffusion.horizon - (n_comp - 1) * self.diffusion.len_ovlp_cd
        )
        ...
        for i_c in range(n_comp):
            ...
            if i_c == 0:
                ...
                ## B,hstep,dim
                trajs_out[:, tmp_idx_1:tmp_idx_2, :] = tjs_p_i[
                    :, : self.hzn_step_size, :
                ]
            elif i_c < n_comp - 1:
        ...
        for i_c in range(n_comp - 1):
            ...
            ## b,len_o,d
            trajs_blend = blend_2_np_trajs_23d(
                end_tjs_i, st_tjs_i_plus_1, self.blend_type, self.exp_beta
            )
...
def blend_2_np_trajs_23d(
    traj_1: np.ndarray, traj_2: np.ndarray, blend_type="exponential", beta=5
):
    """
    ...
    if blend_type in ["exponential", "exp"]:
        ...
        def w(t):
            return 0.5 * (
                1
                + np.cos(
                    np.pi * (t - t_overlap_start) / (t_overlap_end - t_overlap_start)
                )
            )
        ...
        raise ValueError(
            "Invalid blending function. Choose 'exponential', 'cosine', 'linear', or 'smoothstep'."
        )
