...
def load_environment(name):
    ...
    if "hopper" in name or "halfcheetah" in name or "walker" in name:
        ...
        from ..locomo.loco_misc import get_loco_short_env_name
...
def load_env_gym_robo(name_ori):
    ...
    """
    if not isinstance(name_ori, str):
        ## name is already an environment
        return name_ori

    from .gym_robo_utils import get_gym_robo_env_name
    ...
    from .comp.maze2d_constants import get_str_maze_spec
...
def get_dataset(env):
    ...
    if hasattr(env, "get_dataset"):
        ...
        raise AttributeError(
            "Environment does not provide get_dataset() and no h5path was set"
        )
