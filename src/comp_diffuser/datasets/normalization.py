...
class DatasetNormalizer:
    ...
    def __init__(self, dataset, normalizer, eval_solo=False, path_lengths=None):
        if eval_solo:
...
class CDFNormalizer(Normalizer):
    ...
    def __init__(self, X, nm_name=None):
        ...
        self.cdfs = [
            CDFNormalizer1d(self.X[:, i], nm_name=f"{nm_name}_{i}")
...
def empirical_cdf(sample):
    ...
    ## quantitles is already sorted by np.unique
    # find the unique values and their corresponding counts
    quantiles, counts = np.unique(sample, return_counts=True)

    # .size, how many elements inside the array -- cumprod of shape
