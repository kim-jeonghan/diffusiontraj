...
def plot_start_goal(
    ...
    start_outer_circle = plt.Circle(
        (start_x, start_y), size_out_cicle, facecolor="white", edgecolor="black"
    )
    ax.add_patch(start_outer_circle)
    start_inner_circle = plt.Circle(
        (start_x, start_y), size_in_st, color=sg_color
    )  # black
    ax.add_patch(start_inner_circle)

    goal_x, goal_y = start_goal[1]
    goal_outer_circle = plt.Circle(
        (goal_x, goal_y), size_out_cicle, facecolor="white", edgecolor="black"
    )
...
def make_traj_images(
    ...
    assert (
        isinstance(trajs, np.ndarray) and trajs.ndim == 3 and trajs.shape[2] in [4, 2]
    )
    ...
    for batch_idx in range(batch_size):
        ...
        if is_non_keypt is None:
            ...
            ## plot key
            plt.scatter(
                trajs[is_non_k, batch_idx, 0],
                trajs[is_non_k, batch_idx, 1],
                c=c_map[is_non_k],
                cmap="Reds",
            )
            ...
            is_k = ~is_non_k
            plt.scatter(
                trajs[is_k, batch_idx, 0],
                trajs[is_k, batch_idx, 1],
                c=c_map[is_k],
                cmap="Greens",
                marker="*",
                s=70,
            ...
            if sp_pts_1 is not None:
                ...
                plt.scatter(
                    trajs[sp_1_tmp, batch_idx, 0],
                    trajs[sp_1_tmp, batch_idx, 1],
                    c=c_map[sp_1_tmp],
                    cmap="Blues",
                    marker="^",
                    s=180,
            ...
            for i_tj, trajs_p in enumerate(trajs_list):
                ...
                if isinstance(tmp_color_1, str) and tmp_color_1[0].isupper():
                    c_norm = colors.Normalize(
                        vmin=-0.5, vmax=1
                    )  # Start colormap from 20 instead of 0
                    plt.scatter(
                        trajs_p[batch_idx, :, 0],
                        trajs_p[batch_idx, :, 1],
