...
def save_imgs_to_mp4(imgs, save_path, fps=24, n_repeat_first=0, n_rep_list=None):
    ...
    # from comp_diffuser.utils.eval_utils import suppress_warnings, suppress_stdout
    ...
    with imageio.get_writer(save_path, fps=fps) as writer:
        for idx, img in enumerate(imgs):
            ...
            if img.dtype != np.uint8:
                img = (
                    np.clip(img, 0, 1) if img.dtype in [np.float32, np.float64] else img
                )
                img = (
            ...
            if img.ndim != 3 or img.shape[2] not in [1, 3, 4]:
                raise ValueError(
                    f"Invalid image shape at index {idx}: expected 2D or 3D array with 1, 3, or 4 channels."
                )
    ...
    print_color(f"[save_imgs_to_mp4] to {save_path}")
...
def pad_videos_side(vid_list, pad_len):
    ...
    vid_list = np.pad(
        vid_list,
        ((0, 0), (0, 0), (pad_len,) * 2, (pad_len,) * 2, (0, 0)),
        constant_values=255,
    )
    return vid_list
...
def pad_imgs_side(imgs, pad_len, pad_v):
    ...
    """
    imgs = np.pad(
        imgs, ((0, 0), (pad_len,) * 2, (pad_len,) * 2, (0, 0)), constant_values=pad_v
    )
    return imgs
