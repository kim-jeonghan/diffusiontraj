"""
This file is probably not used in CompDiffuser.
"""
...
"""
important: July 23, 2024:
This is already an old rendering code, see maze_renderer.py for the current Maze2D implementation
"""
...
class MazeRenderer:
    ...
    def renders(self, observations, conditions=None, title=None, is_non_keypt=None):
        """
        is_non_keypt: 1d, (H,), (np bool)
        """
        plt.clf()
        fig = plt.gcf()
        ...
        if is_non_keypt is None:
            ...
            # b_size = is_non_keypt.shape[0]
            # assert b_size == observations.shape[0]
            ## B,1 -> B,num_keypts
            # b_indices = np.arange(  b_size )[:, None]
            # b_indices = np.repeat(b_indices, repeats=is_non_keypt.shape[1], axis=1)
            # observations
            plt.scatter(observations[is_non_keypt,1], observations[is_non_keypt,0], 
                            c=colors[is_non_keypt], zorder=20, alpha=0.3, s=40) # s not given
            is_keypt = ~is_non_keypt
            plt.scatter(observations[is_keypt,1], observations[is_keypt,0], 
                            c=colors[is_keypt], zorder=25, marker='*', s=380) #s=400
