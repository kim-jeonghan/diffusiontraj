...
def watch(args_to_watch):
    ...
    def _fn(args):
        ...
        utils.print_color('exp_name', exp_name)
        return exp_name
    return _fn
...
class ArgsParser(Tap):
    ...
    def parse_args(self, experiment=None, from_jupyter=False, use_config_2=False, 
        ...
        # pdb.set_trace() ## if in config file, then overwrite the outside value
    ...
    def add_extras(self, args):
        ...
        for i in range(0, len(extras), 2):
            ...
            val = extras[i+1]
            extra_args_list = ['plan_n_ep', 'diffusion_epoch', 'config_2'] # used in commandline only
    ...
    def set_seed(self, args):
        if 'seed' not in dir(args):
            return
        utils.print_color(f'[ utils/setup ] Setting seed: {args.seed}', c='b')
        set_seed(args.seed)
    ...
    def set_wandb(self, args):
        # 'artifacts/runs/maze2d-large-v1/diffusion/H384_T256_maze2d_test'
    ...
    # def __setattr__(self, name, value):
        ## update the implicit dict as well
