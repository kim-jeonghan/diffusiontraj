    ...
    for i_st, base_st_pos in enumerate(start_pos_list):
        for i_gl, base_gl_pos in enumerate(gl_pos_list):
            for i_ep in range(rs_cfg["n_probs_per_pair"]):
                ...
                tmp_noise_1 = np.random.uniform(
                    size=base_st_pos.shape, low=p_low, high=p_high
                )
                st_pos = base_st_pos + tmp_noise_1

                tmp_noise_2 = np.random.uniform(
                    size=base_gl_pos.shape, low=p_low, high=p_high
                )
                gl_pos = base_gl_pos + tmp_noise_2

                # pdb.set_trace()

                st_state = np.concatenate([st_pos, [0.0, 0.0]])
